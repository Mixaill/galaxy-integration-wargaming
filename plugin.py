...
class WargamingPlugin(Plugin):
    ...
    async def get_local_games(self):
        ...
        if not self._backend_wgc.IsInstalled():
            return local_games

        games = self._backend_wgc.GetInstalledGames()
        for game in games:
            if not game.IsInstalled():
                continue

            local_games.append(LocalGame(game.GetId(), LocalGameState.Installed))
    ...
    async def launch_game(self, game_id):
        game = self._backend_wgc.GetGame(game_id)
        if game is not None:
            game.RunExecutable()
